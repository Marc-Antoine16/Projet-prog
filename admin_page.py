...
class AdminPage(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Seulement les colonnes configurées (pas de row pour éviter les espaces verticaux)
        for i in range(10):
    ...
    def show_users_section(self):
        ...
        self.btn_modifier_mot_passe = ctk.CTkButton(
            self.content, text="Changer mot de passe", command=self.update_password
        )
        self.btn_modifier_mot_passe.grid(row=7, column=5, pady=5, sticky="n")

        self.btn_reset_user = ctk.CTkButton(
            self.content, text="Réinitialiser le compte", fg_color="orange", command=self.reset_user
        )
        self.btn_reset_user.grid(row=8, column=5, pady=10, sticky="n")

        self.btn_supprimer_user = ctk.CTkButton(
            self.content, text="Supprimer l'utilisateur", fg_color="red", command=self.delete_user
        )
    ...
    def show_actions(self):
        ...
        title = ctk.CTkLabel(
            self.content, text=f"Actions détenues par {self.current_user.username}", font=("Arial", 26, "bold")
        )
        title.grid(row=0, column=0, columnspan=10, pady=(50, 20), sticky="n")

        if not self.current_user.stocks_owned:
            ctk.CTkLabel(self.content, text="Aucune action détenue actuellement.", text_color="gray").grid(
        ...
        for i, (symbol, data) in enumerate(self.current_user.stocks_owned.items(), start=2):
            ...
            ctk.CTkLabel(frame, text=info, font=("Arial", 16)).grid(row=0, column=0, padx=10, pady=5)
    ...
    def show_watchlist(self):
        ...
        title = ctk.CTkLabel(
            self.content, text=f"Watchlist de {self.current_user.username}", font=("Arial", 26, "bold")
        )
        title.grid(row=0, column=0, columnspan=10, pady=(50, 20), sticky="n")
        ...
        if not self.current_user.watchlist:
            ctk.CTkLabel(self.content, text="Aucune action dans la watchlist.", text_color="gray").grid(
        ...
        for i, stock in enumerate(self.current_user.watchlist, start=3):
            ...
            ctk.CTkButton(
                frame, text="Supprimer", fg_color="red", command=lambda s=stock: self.remove_from_watchlist(s)
    ...
    def load_user(self):
        ...
        for user in self.users:
            if user["username"] == username:
                ...
                break
        self.user_info.configure(
            text=f"Utilisateur chargé : {self.current_user.username}\nSolde actuel : {self.current_user.balance:.2f} $",
            text_color="white",
        )
    ...
    def delete_user(self):
        ...
        with open("users.json", "r") as f:
            users = json.load(f)
        new_users = [user for user in users if user["username"] != self.current_user.username]
        ...
        self.user_info.configure(
            text=f"Utilisateur {self.current_user.username} supprimé avec succès.", text_color="green"
        )
